...
这是**阶段性控制**的核心设计环节。根据业务经验中的"关键流程"，将完整的演练分解为多个典型任务阶段：

**阶段性控制的设计原理**：
- 每个任务（阶段）对应业务流程的一个典型环节
- 每个阶段有明确的业务目标和训练重点
- AI的演绎被精确控制在当前阶段内，防止跨阶段"串场"
- 阶段按业务流程顺序排列，符合行业成熟的流程框架

**为什么需要阶段性控制**：
1. **防止AI跨阶段行为**：
   - 没有阶段划分时，AI可能在"需求探索"阶段就开始报价
   - 明确的阶段边界，将AI限制在当前阶段的范围内
...
阶段3：解决方案介绍
  - 业务目标：针对客户需求介绍产品/方案优势
  - 训练重点：学员的针对性介绍、价值阐述能力
  - AI行为范围：根据介绍精准度表现不同程度兴趣

阶段4：异议处理
  - 业务目标：有效应对客户的疑虑和反对意见
  - 训练重点：学员的异议处理技巧、说服能力
  - AI行为范围：提出典型异议（价格、质量、竞品等）
...
**阶段分解的关键原则**：
1. **顺序性**：符合实际业务流程的顺序
2. **独立性**：每个阶段有独立的目标和控制策略
3. **完整性**：覆盖完整的业务流程
4. **边界清晰**：明确每个阶段的开始和结束条件
5. **数量适中**：通常3-6个阶段（过少不够精细，过多太复杂）

**阶段边界控制机制**：
- 编剧Agent会为每个阶段定义"阶段完成标准"
- AI只有在当前阶段达到完成标准后，才能进入下一阶段
- 如果学员试图跨阶段（如在需求探索时就问价格），AI会将学员拉回当前阶段
- 例如AI回应："价格的事情我们后面再聊，现在我想先了解一下你们的具体需求..."
...
范围控制层是分层控制机制的第二层，它定义AI在每个阶段"怎么想"。

**关键特征：阶段独立性**
- **每个阶段有独立的范围控制层**
- 阶段1的思维原则 ≠ 阶段2的思维原则
- AI进入新阶段时，自动切换到该阶段的思维原则
- 这确保AI的思考方式符合当前阶段的业务目标

对每个阶段（从任务分解中得到），编剧Agent需要生成：
...
- **每个阶段有独立的细致控制层（行为响应表）**
- 阶段1的响应表 ≠ 阶段2的响应表
- AI进入新阶段时，自动切换到该阶段的行为响应表
- 这确保AI的具体响应符合当前阶段的业务逻辑

例如：
- **需求探索阶段**的响应表：学员空泛表述 → AI追问需求细节
- **异议处理阶段**的响应表：学员空泛表述 → AI提出具体质疑

同样是"空泛表述"，在不同阶段AI的响应策略完全不同。
...
完成所有阶段的三层控制生成后，编剧Agent组装完整剧本，形成**阶段性控制×分层控制**的二维协同机制：
...
**协同工作机制**：
1. **全局控制层**：贯穿所有阶段，持续约束AI的基础身份
2. **阶段性控制**：横向限定AI"当前在哪个阶段"
3. **分层控制**：纵向约束AI"在该阶段怎么做"
4. **任何时刻**，AI的行为都受到：全局控制 + 当前阶段的范围控制 + 当前阶段的细致控制
